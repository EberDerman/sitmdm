...
$tecnicaturas = $tecnicaturaRepository->getTecnicaturas();
?>
<title>Ingresar Técnicas</title>
<script>
  ...
  <main>
    ...
    <div class="container card text-center my-3 px-0">
      <div class="card-header bg-primary text-white d-flex justify-content-between align-items-center py-3 mb-4"
        ...
        <h4 class="mb-0">Seleccionar tecnicatura para visualizar alumnos</h4>
        <div>
      ...
      <div class="card-body">
        <div class="table-responsive text-nowrap">
          <table class="table table-bordered table-hover table-striped display AllDataTables" cellspacing="0"
            ...
            <tbody>
              <?php
              foreach ($tecnicaturas as $row) {
                $id_Tecnicatura = $row['id_Tecnicatura'];
                ?>
                <tr>
                  <td><?php echo $row['id_Tecnicatura']; ?></td>
                  <td><?php echo $nombreTec= $row['nombreTec']; ?></td>
                  <td><?php echo $Resolucion = $row['Resolucion']; ?></td>
                  <td><?php echo $Ciclo = $row['Ciclo']; ?></td>
                  <td>
                    <a style='color: #55acee'
                      href='precepEstudiantesVer.php?id_Tecnicatura=<?= $id_Tecnicatura ?>&nombreTec=<?= $nombreTec ?>&Ciclo=<?= $Ciclo ?>'
