...
if ($action == 'nueva_asistencia') {
    ...
    $stmt_check->close();
} elseif ($action == 'update_nota') {
    $column = $_POST['column'];
    $value = $_POST['value'];

    $sql_update = "UPDATE estudiante_materia SET $column = ? WHERE id_estudiante = ? AND id_Materia = ?";
    $stmt_update = $conexion->prepare($sql_update);
    $stmt_update->bind_param("sii", $value, $id_estudiante, $id_Materia);

    echo $stmt_update->execute() ? "Nota actualizada" : "Error al actualizar la nota";
    $stmt_update->close();
}
